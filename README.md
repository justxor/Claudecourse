...
5. [Базовый агентный цикл (сердце всего)](#5-базовый-агентный-цикл-сердце-всего)
   - [5A. Что такое харнесс (агентная обвязка)](#5a-что-такое-харнесс-агентная-обвязка)
...
## 5A. Что такое харнесс (агентная обвязка)
...
### Зачем нужен харнесс: 5 проблем голого цикла
...
### Из чего собран харнесс: 6 слоёв по шагам

Читайте сверху вниз — так обвязка «наматывается» на цикл слой за слоем.

1. **Сборка входа.** Разобрать `/команды`, склеить системный промпт (инструменты + CLAUDE.md), нормализовать историю. Именно здесь цикл получает контекст, а не только голый вопрос.
2. **Ворота прав.** Перед каждым вызовом инструмента — хуки, правила `allow/deny`, при необходимости вопрос пользователю. Опасное блокируется до выполнения.
3. **Исполнение и устойчивость.** Вызов инструмента с таймаутом; транзиентные ошибки (429/500) — ретраятся, результат (в т.ч. ошибка) возвращается в цикл как `tool_result`.
4. **Управление контекстом.** Следим за бюджетом токенов; при переполнении — сжатие старой истории, чтобы цикл не «ослеп».
5. **Персистентность.** Каждый ход пишется в JSONL синхронно — сессию можно продолжить (`--continue`) даже после падения.
6. **Наблюдаемость и вывод.** Стриминг ответа в терминал, лог каждого витка (модель, токены, решение прав, длительность), финальная стоимость.
...
### Минимальный харнесс своими руками
...
def call_api_with_retry(messages, tools, attempts=4):
    ...
    raise RuntimeError("API недоступен после ретраев")
```
...
### Трассировка прогона с харнессом (виток за витком)
...
### Как это связано с остальным гайдом
...
### Учебный агент vs продакшн-харнесс
...
### Чек-лист: минимальный харнесс за один день
...
### Антипаттерны харнесса: как НЕ надо
